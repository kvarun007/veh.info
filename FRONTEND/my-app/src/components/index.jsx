	...
	const fetchApiData = async () => {
		try {
			...
			const response = await fetch(
				`http://127.0.0.1:8000/bike/${searchValue}/`
			);
		...
		<>
			<div className="bg-[#FFCF9D] h-screen ">
				...
				{data.bikeImage ? (
					<div className="flex justify-center">
						<img
							src={`http://127.0.0.1:8000/${data.bikeImage}`}
							alt="Bike"
							className="mt-4 w-[32rem] border border-indigo-600 rounded-"
						/>
					</div>
				) : null}
