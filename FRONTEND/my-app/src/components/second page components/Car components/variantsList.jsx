...
export default function VariantsList({ data }) {
	...
	return (
		<Box className="px-6 pt-4 bg-gray-50  max-h-svh overflow-y-auto">
			{data.map((vehicle, index) => (
				<Card
					key={index}
					className="mb-6 shadow-md border border-gray-200 rounded-lg transition-transform transform hover:shadow-lg hover:-translate-y-1"
				>
					<CardContent className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 p-6">
						...
						<Box className="flex-1">
							<Typography
								variant="h6"
								component="h2"
								className="text-gray-900 font-bold mb-2 text-xl"
							>
						...
						<Box className="flex flex-col items-end">
							<Typography
								variant="h6"
								component="p"
								className="text-gray-800 font-bold mb-2 text-2xl"
							>
